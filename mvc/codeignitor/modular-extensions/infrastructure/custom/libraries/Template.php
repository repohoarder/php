...
class Template
{
	...
	public function build($view, $data = array(), $return = FALSE)
	{
		...
		if ( ! $return && class_exists('Modules') && Modules::$module_ran):
	...
	public function set_layout($view, $_layout_subdir = '')
	{
		...
		if (class_exists('Modules') && Modules::$module_ran):
	...
	/**
	 * Put extra javascipt tags before others prior to the end body tag
	 *
	 * @access	public
	 * @param	 string	$line	The line being added
	 * @return	void
	 */
	public function prepend_footermeta($line)
	{
		array_unshift($this->_footer_meta, $line);

		return $this;
	}

	/**
	 * Put extra javascipt tags after other meta tags
	 *
	 * @access	public
	 * @param	 string	$line	The line being added
	 * @return	void
	 */
	public function append_footermeta($line)
	{
		$this->_footer_meta[] = $line;
	 ...
	 * @return	void
	 */
	public function set_footermeta($name, $content, $type = 'meta')
	{
		$name = htmlspecialchars(strip_tags($name));
		$content = htmlspecialchars(strip_tags($content));
		...
		{
			case 'meta':
				$this->_footer_meta[$name] = '<meta name="'.$name.'" content="'.$content.'" />';
			break;

			case 'link':
				$this->_footer_meta[$content] = '<link rel="'.$name.'" href="'.$content.'" />';
			break;
		}
