...
class Pay_By_Dept extends MX_Controller {
	...
	function index($brand = 'all_brands')
	{
		...
		$response = $this->platform->post('crm/reports/payroller/pay_by_dept/'.$brand, $params);
		...
		foreach ($rdata as $name => $series):
			...
			$all_series[] = array(
				'name' => $series_array['department_name'],
				'data' => $series_data
			);
