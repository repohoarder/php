...
class Rev_aff_rev extends MX_Controller {
	...
	function index($brand = 'brain_host')
	{
		...
		$response = $this->platform->post('crm/reports/affiliate/total/'.$brand, $params);
		if ( ! $response || ! array_key_exists('success',$response) || ! $response['success']):

			return;
		endif;
		$rdata['Total']=$response['data'][$brand];
		...
		$rdata['Affiliate']=$response['data'][$brand];
		
		$response = $this->platform->post('crm/reports/affiliate/traditional/'.$brand, $params);
		if ( ! $response || ! array_key_exists('success',$response) || ! $response['success']):

			return;
		endif;
		$rdata['Traditional']=$response['data'][$brand];
		
		$data['title']          = 'Traditional vs. Affiliate Revenue';
