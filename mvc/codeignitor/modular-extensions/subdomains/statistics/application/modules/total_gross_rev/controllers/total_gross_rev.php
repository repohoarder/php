...
class Total_gross_rev extends MX_Controller {
	...
	function index($brand = 'all_brands')
	{
		...
		else
		{
			...
			$response = $this->platform->post('crm/reports/revenue/gross/'.$brand, $params);
			if ( ! $response || ! array_key_exists('success',$response) || ! $response['success']):

				return;
			endif;
			$rdata['Gross']=$response['data'][$brand];
		}
		
		
		
		$data['title']               = 'Total Gross Revenue';
