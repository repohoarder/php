...
class Renewal_count extends MX_Controller {
	...
	function index($brand = 'all_brands')
	{
		...
		if ($brand=='all_brands')
		{
			$response = $this->platform->post('crm/reports/renewals/count/brainhost', $params);
			...
			$response = $this->platform->post('crm/reports/renewals/count/brazil_orders', $params);
			if ( ! $response || ! array_key_exists('success',$response) || ! $response['success']):
			...
			$response = $this->platform->post('crm/reports/renewals/count/purelyhosting', $params);
			if ( ! $response || ! array_key_exists('success',$response) || ! $response['success']):
		...
		else
		{
			...
			$response = $this->platform->post('crm/reports/renewals/count/'.$brand, $params);
			if ( ! $response || ! array_key_exists('success',$response) || ! $response['success']):

				return;
			endif;
			$rdata['Renewals Count']=$response['data'][$brand];
		}
		
		
		
		$data['title']               = 'Renewals Count';
