...
class Total_visits extends MX_Controller {
	...
	function index($brand = 'all_brands')
	{
		...
		foreach ($show_brands as $key=>$val):
			...
			$brands[$val] = $this->platform->post('google_api/visits/get/'.$key, $params);

		endforeach;
		
		//$brands['Traditional'] = $this->platform->post('affiliate_software/reports/traffic/traditional', $params);
		...
		foreach ($response['data'] as $name => $series):
			...
			foreach ($series as $series_array):
				...
				$series_data[] = $series_array['amount'];
