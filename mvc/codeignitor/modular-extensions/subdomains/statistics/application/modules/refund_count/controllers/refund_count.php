...
class Refund_count extends MX_Controller {
	...
	function index($brand = 'all_brands')
	{
		...
		if ($brand=='all_brands')
		{
			$response = $this->platform->post('crm/reports/refund/count/brainhost', $params);
			...
			$response = $this->platform->post('crm/reports/refund/count/brazil_orders', $params);
			if ( ! $response || ! array_key_exists('success',$response) || ! $response['success']):
			...
			$response = $this->platform->post('crm/reports/refund/count/purelyhosting', $params);
			if ( ! $response || ! array_key_exists('success',$response) || ! $response['success']):
		...
		else
		{
			...
			$response = $this->platform->post('crm/reports/refund/count/'.$brand, $params);
			if ( ! $response || ! array_key_exists('success',$response) || ! $response['success']):

				return;
			endif;
			$rdata['Refund Count']=$response['data'][$brand];
		}
		
		
		
		$data['title']               = 'Refund Count';
