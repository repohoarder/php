...
class Renewal_revenue extends MX_Controller {
	...
	function index($brand = 'all_brands')
	{
		...
		if ($brand=='all_brands')
		{
			$response = $this->platform->post('crm/reports/renewals/revenue/brainhost', $params);
			...
			$response = $this->platform->post('crm/reports/renewals/revenue/brazil_orders', $params);
			if ( ! $response || ! array_key_exists('success',$response) || ! $response['success']):
			...
			$response = $this->platform->post('crm/reports/renewals/revenue/purelyhosting', $params);
			if ( ! $response || ! array_key_exists('success',$response) || ! $response['success']):
		...
		else
		{
			...
			$response = $this->platform->post('crm/reports/renewals/revenue/'.$brand, $params);
			if ( ! $response || ! array_key_exists('success',$response) || ! $response['success']):
