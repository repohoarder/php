...
class Paypal_sales extends MX_Controller {
	...
	function index($brand = 'all_brands')
	{
		...
		if ($brand=='all_brands')
		{
			$response = $this->platform->post('crm/reports/paypal_orders/sales/brainhost', $params);
			...
			$response = $this->platform->post('crm/reports/paypal_orders/sales/brazil_orders', $params);
			if ( ! $response || ! array_key_exists('success',$response) || ! $response['success']):
			...
			$response = $this->platform->post('crm/reports/paypal_orders/sales/purelyhosting', $params);
			if ( ! $response || ! array_key_exists('success',$response) || ! $response['success']):
		...
		else
		{
			...
			$response = $this->platform->post('crm/reports/paypal_orders/sales/'.$brand, $params);
			if ( ! $response || ! array_key_exists('success',$response) || ! $response['success']):

				return;
			endif;
			$rdata['Sales']=$response['data'][$brand];
		}
		
		
		
		$data['title']               = 'Paypal Sales';
