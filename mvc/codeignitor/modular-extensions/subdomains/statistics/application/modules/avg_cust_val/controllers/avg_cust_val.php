...
class Avg_cust_val extends MX_Controller {
	...
	function index($brand = 'all_brands')
	{
		...
		// BRANDS PLATFORM CALL
			...
			foreach ($show_brands as $key=>$val):
				...
				$params['brand'] = $key;
				$brands[$val] = $this->platform->post('crm/reports/sales/avg_cust_val_orders', $params);
				$averages[$val] = $this->platform->post('crm/reports/sales/avg_cust_val_average', $params);
		...
		$data['title']          = 'Average Order Totals';
