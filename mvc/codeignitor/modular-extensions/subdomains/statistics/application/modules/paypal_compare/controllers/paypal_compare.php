...
class Paypal_compare extends MX_Controller {
	...
	function index($brand = 'all_brands')
	{
		...
		else
		{
			...
			$response = $this->platform->post('crm/reports/paypal_orders/attempted/'.$brand, $params);
			if ( ! $response || ! array_key_exists('success',$response) || ! $response['success']):

				return;
			endif;
			$rdata['Attempted']=$response['data'][$brand];
			
			$response = $this->platform->post('crm/reports/paypal_orders/charged/'.$brand, $params);
			if ( ! $response || ! array_key_exists('success',$response) || ! $response['success']):
		...
		$data['title']               = 'Paypal Attempted vs Charged';
