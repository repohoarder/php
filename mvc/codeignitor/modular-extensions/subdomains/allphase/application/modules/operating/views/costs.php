		...
		<div class="module s-sales">
			...
			<table id="pnl-basic-stats">
				...
				<tbody>
					...
					$fees_ch = round($processing+$hosting_fees+$domain_fees+$upsell_fees+$call_fee+$tick_fee,2);
					$expenses = round($refunds+$chargebacks+$fees_ch+$reserves+$freebies,2);
					...
					<tr>
						...
						<td>$<?php echo $net;?></td>
						<td>$<?php echo number_format($refunds+$chargebacks ,2); ?></td>
			...
			<form action="/operating/costs" method="post">
			...
			<div id="pnl-custom-stats" style="display:block;">
				<table>
					...
					<tbody>
							...
							$tick_fee		= ! isset($range['ticketfee']['total']['replies'])? 0.00 :round($range['ticketfee']['total']['replies'],2);
							$freebies		= ! isset($range['freebie']['total']['amount'])? 0.00 :round($range['freebie']['total']['amount'],2);
						
							$fees = round($processing+$hosting_fees+$domain_fees+$upsell_fees+$call_fee+$tick_fee,2);
							$expenses = round($refunds+$chargebacks+$fees+$reserves+$freebies,2);
							$net = round($revenue - $expenses,2);
							if($net < 0):
								$net = $net * -1;
								$net = "-".number_format($net,2);
